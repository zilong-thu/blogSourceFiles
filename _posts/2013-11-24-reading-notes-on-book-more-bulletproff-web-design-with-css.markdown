...
<p style="font-weight:bold;font-size:1.1em;line-height:1.1em;">好了，下面开始从头阅读此书。</p>
...
> +首先导入重置样式表，确保基础环境的统一，并且避免重复写样式。
> +在`master.css`、`ie.css`中加入清除浮动的声明，命名为`.group`类，以便需要时随时调用，可在所有浏览器中清除容器内的浮动。
> +用单独的样式表隔离针对IE的破解和补丁。
